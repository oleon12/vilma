...
<br>
  ...
  <img src="../inst/app/www/Trimeresurus_folder.png"
       alt="Fig. 1"
...
path_t <- "/media/omar/Extreme Pro/Projects/vilma_doc/Trimeresurus/"

shp_files <- paste0(path_t, shp_files)
...
str(list_of_sf, list.len = 2, max.level = 1)
```
<br>
...
<br>
```{r, warning=FALSE, message=FALSE, fig.align='center', eval=run_terra}
library(tmap)

# Check one of the polygons
tm_shape(list_of_sf[[1]])+ # Plot the polygon
  tm_polygons()
...
```{r, message=FALSE, warning=FALSE, eval=FALSE, eval=run_heavy}
library(vilma)
...
<br>
```{r, eval=run_terra}
# Check one of the rasters
tm_shape(trimeresurus_out$rasters[[1]])+
  tm_raster()+
tm_shape(list_of_sf[[1]])+
