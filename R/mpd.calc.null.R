...
mpd.calc.null <- function(pd, tree, dist, iterations = 999,
  ...
  if(sampling == "taxa.label"){
    ...
    for(i in 1:iterations){
      ...
      #Calculate pd
      pd.iter <- NA
      pd.iter <- suppressMessages(mpd.calc(iter.tree.out[[i]], dist1, method = pd$calculation.method, abundance = pd$abundance))
  ...
  if(sampling  == "range"){
    ...
    for(i in 1:iterations){
      ...
      pd.iter <- NA
      pd.iter <- suppressMessages(mpd.calc(tree, iter.dist[[i]], method = pd$calculation.method, abundance = pd$abundance))
  ...
  if(sampling == "regional"){
    ...
    for(i in seq_len(iterations)){
      # compute PD
      pd_null <- suppressMessages(mpd.calc(tree, iter.dist[[i]], method = pd$calculation.method, abundance = pd$abundance))
