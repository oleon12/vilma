...
write.vilma.beta <- function(vilma.beta, file, raster.format = c("tif","grd","img"), overwrite = TRUE){
  ...
  if (!inherits(vilma.beta, "vilma.beta")){
    stop("Input is not a vilma.beta object.")
}
  ...
  if(vilma.beta$algorithm == "PhyloSor"){
    ...
    for(i in seq_along(r.names)){
      suppressMessages(writeRaster(x = vilma.beta$rasters[[i]], filename = r.names[[i]], overwrite = overwrite))
    }
