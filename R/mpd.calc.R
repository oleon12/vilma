...
#' @description
#' Computes the Mean Pairwise Distance (MPD) among species within spatial cells
...
#'
#' @details
#' MPD is calculated as the average phylogenetic distance between all possible pairs of species 
#' in a community (cell). When \code{abundance = TRUE}, MPD is weighted by the relative abundance
#' of each species.  
...
#'   \item \code{rasters} – List with abundance, richness, and MPD rasters.
...
#'   \item \code{index} – The string \code{"mpd.calc"}.
#' }
#'
#' @references
...
#'
#' @seealso \code{\link{faith.pd}}, \code{\link{pd.taxon}}, \code{\link{points_to_raster}}
#'
#' @examples
#' \dontrun{
#' tree <- examplae_tree()
...
mpd.calc <- function(tree, dist, method = c("root","node","exclude"), abundance = FALSE){
  ...
  for(i in 1:length(rownames(pd.res))){
    ...
    if(length(sp.list) > 1){
      ...
      if (abundance == TRUE) {
        ...
        ## --- alignment added (only change) ---
        rel_ab <- rel_ab[rownames(co.dist)]
        rel_ab[is.na(rel_ab)] <- 0
        rel_ab <- rel_ab / sum(rel_ab)
        ## -------------------------------------

        # Outer product of abundances
        weights <- outer(rel_ab, rel_ab)
        diag(weights) <- 0
  
        # Weighted MPD: use only upper triangle to avoid double counting
        num <- sum(co.dist[upper.tri(co.dist)] * weights[upper.tri(weights)])
        den <- sum(weights[upper.tri(weights)])
  
        PD <- c(PD, num / den)
  
        } else {
          co.dist <- co.dist[upper.tri(co.dist)]
          PD <- c(PD, mean(co.dist))
        }
  ...
  }  # <-- closes the for(i in 1:length(rownames(pd.res)))
