...
#' pe <- pe.calc(tree, dist)
#' null_model <- pe.calc.null(pe, tree, dist)
#' print(null_model)
#' plot(null_model)
...
pe.calc.null <- function(pd, tree, dist, iterations = 999, 
  ...
  if (!is.rooted(tree)) {
    stop("PE requires a rooted tree.")
  }
 ...
 if(length(regional.weight) != 1){
  ...
  if(sampling == "taxa.label"){
    ...
    for(i in 1:iterations){
      ...
      #Calculate pd
      pd.iter <- NA
      pd.iter <- suppressMessages(pe.calc(iter.tree.out[[i]], dist1))
  ...
  if(sampling  == "range"){
    ...
    for(i in 1:iterations){
      ...
      pd.iter <- NA
      pd.iter <- suppressMessages(pe.calc(tree, iter.dist[[i]]))
  ...
  if(sampling == "regional"){
    ...
    for(i in seq_len(iterations)){
      # compute PD
      pd_null <- suppressMessages(pe.calc(tree, iter.dist[[i]]))
