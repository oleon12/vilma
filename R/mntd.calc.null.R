...
#'
#' @seealso \code{\link{mntd.calc}}, \code{\link{points_to_raster}}, 
#'
#' @examples
#' \dontrun{
#' tree <- examplae_tree()
...
mntd.calc.null <- function(pd, tree, dist, iterations = 999,
  ...
  if(sampling == "taxa.label"){
    ...
    for(i in 1:iterations){
      ...
      #Calculate pd
      pd.iter <- NA
      pd.iter <- suppressMessages(mntd.calc(iter.tree.out[[i]], dist1, method = pd$calculation.method, abundance = pd$abundance))
  ...
  if(sampling  == "range"){
    ...
    for(i in 1:iterations){
      ...
      pd.iter <- NA
      pd.iter <- suppressMessages(mntd.calc(tree, iter.dist[[i]], method = pd$calculation.method, abundance = pd$abundance))
  ...
  if(sampling == "regional"){
    ...
    for(i in seq_len(iterations)){
      # compute PD
      pd_null <- suppressMessages(mntd.calc(tree, iter.dist[[i]], method = pd$calculation.method, abundance = pd$abundance))
